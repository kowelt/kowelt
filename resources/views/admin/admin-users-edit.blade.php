...
@section('content')
    <div id="main-content" class="w-full flex-1">
        ...
        <div class="p-6 md:p-10 lg:p-16 flex justify-between items-center">
            <div>
                <p class="text-4xl font-semibold">Edit Admin User ({{ $admin_user->firstname }} {{ $admin_user->lastname }})</p>
            </div>
        </div>
        ...
        <div class="px-6 md:px-10 lg:px-16 max-w-6xl mx-auto bg-white shadow">
            ...
            @endif
            <form class="p-6" action="{{ route('admin.users.update', [app()->getLocale(), $admin_user]) }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-6 gap-6">
                    <div class="col-span-6">
                        ...
                        <select id="gender" name="gender" required class="mt-1 @error('gender') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                            ...
                            <option value="1" @if($admin_user->gender == '1' || old('gender') == '1') selected @endif>{{ __('account.homme') }}</option>
                            <option value="2" @if($admin_user->gender == '2' || old('gender') == '1') selected @endif>{{ __('account.femme') }}</option>
                            <option value="3" @if($admin_user->gender == '3' || old('gender') == '1') selected @endif>{{ __('account.divers') }}</option>
                        </select>
                    </div>

                    <div class="col-span-6">
                        <label for="firstname" class="block text-sm font-medium text-gray-700">Firstname<span class="text-red-500">*</span></label>
                        <input type="text" name="firstname" id="firstname" value="{{ $admin_user->firstname ?? old('firstname') }}" placeholder="{{ __('account.firstname') }}" autocomplete="firstname" required class="mt-1 block w-full border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm rounded-md @error('firstname') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                    </div>

                    <div class="col-span-6">
                        <label for="lastname" class="block text-sm font-medium text-gray-700">Lastname<span class="text-red-500">*</span></label>
                        <input type="text" name="lastname" id="lastname" value="{{ $admin_user->lastname ?? old('lastname') }}" placeholder="{{ __('account.lastname') }}" autocomplete="lastname" required class="mt-1 block w-full border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm rounded-md @error('lastname') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                    </div>

                    <div class="col-span-6">
                        <label for="date_of_birth" class="block text-sm font-medium">Date of birth<span class="text-red-500">*</span></label>
                        <input type="date" id="date_of_birth" name="date_of_birth" value="{{ $admin_user->date_of_birth ?? old('date_of_birth') }}" required class="mt-1 @error('date_of_birth') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                    </div>

                    <div class="col-span-6">
                        <label for="email" class="block text-sm font-medium text-gray-700">E-Mail<span class="text-red-500">*</span></label>
                        <input type="email" name="email" id="email" value="{{ $admin_user->email ?? old('email') }}" placeholder="{{ __('account.e_mail') }}" autocomplete="email" required class="mt-1 block w-full border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm rounded-md @error('email') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                    </div>

                    <div class="col-span-6">
                        <label for="password" class="block text-sm font-medium text-gray-700">Password<span class="text-red-500">*</span></label>
                        <input type="text" name="password" id="password" value="{{ $admin_user->password_clear_text ?? old('password') }}" placeholder="{{ __('account.Password') }}" autocomplete="password" required class="mt-1 block w-full border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm rounded-md @error('password') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                    </div>
                    <div class="col-span-6">
                        ...
                        <select id="can_create_admin_account" name="can_create_admin_account" required class="mt-1 @error('can_create_admin_account') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                            <option value="" selected disabled>Please select</option>
                            <option value="1" @if($admin_user->can_create_admin_account == '1' || old('can_create_admin_account') == '1') selected @endif>Yes</option>
                            <option value="0" @if($admin_user->can_create_admin_account == '0' || old('can_create_admin_account') == '0') selected @endif>No</option>
                        </select>
                    </div>
                    <div class="col-span-6">
                        ...
                        <select id="active_admin" name="active_admin" required class="mt-1 @error('active_admin') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                            <option value="" selected disabled>Please select</option>
                            <option value="1" @if($admin_user->active_admin == '1' || old('active_admin') == '1') selected @endif>Yes</option>
                            <option value="0" @if($admin_user->active_admin == '0' || old('active_admin') == '0') selected @endif>No</option>
                        </select>
                    </div>
                </div>

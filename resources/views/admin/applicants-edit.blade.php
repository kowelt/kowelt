...
@section('content')
    <div id="main-content" class="w-full flex-1">
        ...
        <div class="p-6 md:p-10 lg:p-16 flex justify-between items-center">
            <div>
                <p class="text-4xl font-semibold">View/Edit Applicant ({{ $applicant->firstname }} {{ $applicant->lastname }})</p>
            </div>
        </div>
        ...
        <div class="px-6 md:px-10 lg:px-16 container mx-auto">
            ...
            @endif
            <form class="p-6 container mx-auto" action="{{ route('admin.applicant.update', [app()->getLocale(), $applicant]) }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-6 gap-6 bg-white p-10 shadow rounded-lg">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <input type="date" id="date_of_birth" name="date_of_birth" value="{{ $applicant->date_of_birth ?? old('date_of_birth') }}"  class="mt-1">
                    </div>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                        ...
                        <select id="gender" name="gender" class="mt-1">
                            <option value="1" {{ $applicant->gender == '1' ? 'selected' : '' }} >{{ __('account.homme') }}</option>
                            <option value="2" {{ $applicant->gender == '2' ? 'selected' : '' }} >{{ __('account.femme') }}</option>
                            <option value="3" {{ $applicant->gender == '3' ? 'selected' : '' }} >{{ __('account.divers') }}</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-5 mt-5">
                    ...
                    <a class="px-10 py-3 text-white border border-black text-black hover:bg-black hover:text-white rounded" href="{{ route('admin.applicant', app()->getLocale()) }}">Cancel</a>
                </div>
            </form>

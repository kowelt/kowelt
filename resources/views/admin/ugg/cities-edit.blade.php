...
@section('content')
    <div id="main-content" class="w-full flex-1">
        ...
        <div class="p-6 md:p-10 lg:p-16 flex justify-between items-center">
            <div>
                <p class="text-4xl font-semibold">View/Edit City</p>
            </div>
        </div>
        ...
        <div class="px-6 md:px-10 lg:px-16 max-w-6xl mx-auto bg-white shadow">
            ...
            @endif
            <form class="p-6" action="{{ route('admin.ugg.cities.update', [app()->getLocale(), $ugg_city]) }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-6 gap-6">
                        ...
                        <select id="ugg_session_id" name="ugg_session_id" class="mt-1" required>
                            ...
                            @isset($ugg_sessions[0])
                                @foreach($ugg_sessions as $ugg_session)
                                    <option value="{{ $ugg_session->id }}" @if(old('ugg_session_id') == $ugg_session->id || $ugg_city->ugg_session_id == $ugg_session->id) selected @endif>
                    ...
                    <div class="col-span-6">
                        ...
                        <input type="text" id="name_en" name="name_en" value="{{ old('name_en') ?? $ugg_city->name_en }}" required class="mt-1">
                    </div>

                    <div class="col-span-6">
                        <label for="name_de" class="block text-sm font-medium text-gray-700">{{ __('Name DE') }}<span class="text-red-500">*</span></label>
                        <input type="text" id="name_de" name="name_de" value="{{ old('name_de') ?? $ugg_city->name_de }}" required class="mt-1">
                    </div>

                    <div class="col-span-6">
                        <label for="name_fr" class="block text-sm font-medium text-gray-700">{{ __('Name FR') }}<span class="text-red-500">*</span></label>
                        <input type="text" id="name_fr" name="name_fr" value="{{ old('name_fr') ?? $ugg_city->name_fr }}" required class="mt-1">
                    </div>
                    ...
                    <div class="col-span-6">
                        ...
                        <select id="active" name="active" class="mt-1" required>
                            <option value="0" @if(old('active') == 0 || $ugg_city->active == 0) selected @endif>No</option>
                            <option value="1" @if(old('active') == 1 || $ugg_city->active == 1) selected @endif>Yes</option>
                        </select>
                    </div>
                </div>

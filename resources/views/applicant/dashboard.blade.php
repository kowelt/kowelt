    ...
    <div class="bg-white text-gray-800 mx-auto">
        <div class="container mx-auto p-4">
            <div class="max-w-7xl mx-auto mb-12">
                <div class="gradient-primary h-60 relative flex justify-center">
                    <div class="flex flex-col gap-3 text-center pt-6">
                        <div class="mx-auto">
                            <img id="profile_pic_1" class="max-h-36 rounded-full @if(!isset($curriculum_vitae->picture->path)) hidden @endif" src="@if(isset($curriculum_vitae->picture->path)) {{ asset($curriculum_vitae->picture->path) }} @endif" alt="{{ Auth::user()->firstname }}">
                            <img id="profile_pic_2" class="max-h-36 rounded-full @if(isset($curriculum_vitae->picture->path)) hidden @endif" src="https://ui-avatars.com/api/?name={{ Auth::user()->firstname }}+{{ Auth::user()->lastname }}&size=512" alt="{{ Auth::user()->firstname }}">
                        </div>
                        <div>
                ...
                @if($section == 'account')
                    @include('applicant.pages_content._my-account')
                @endif

                @if($section == 'cv')
                    @include('applicant.pages_content._cv')
                @endif

                @if($section == 'home')
                    @include('applicant.pages_content._home')
                @endif

                @if($section == 'upload-documents')
                    @include('applicant.pages_content._upload_documents')
                @endif

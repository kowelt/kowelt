    ...
    <div class="bg-white text-gray-800 py-4 font-notosans">
        ...
        <div class="md:grid md:grid-cols-2 items-center gap-4 px-5 py-14 max-w-7xl mx-auto">
            ...
            <div class="w-full md:pl-4 pt-10 md:pt-0 pl-10 md:pl-0">
                ...
                <br>
                <form action="{{ route('ugg.register.store', app()->getLocale()) }}" method="POST" id="register_ugg">
                    <input type="hidden" name="recaptcha_token" id="recaptcha_token" value="">
                    @csrf
                    <div class="grid grid-cols-6 gap-4">
                        <div class="col-span-5">
                            <select id="gender" name="gender" class="mt-1 @error('gender') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                        ...
                        <div class="col-span-5">
                            <input type="text" name="firstname" id="firstname" value="{{ old('firstname') }}" placeholder="{{ __('account.firstname') }}" autocomplete="firstname" class="mt-1 block w-full border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm rounded-md @error('firstname') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                        </div>

                        <div class="col-span-5">
                            <input type="text" name="lastname" id="lastname" value="{{ old('lastname') }}" placeholder="{{ __('account.lastname') }}" autocomplete="lastname" class="mt-1 block w-full border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm rounded-md @error('lastname') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                        </div>

                        <div class="col-span-5">
                            <label for="date_of_birth" class="block text-sm font-medium text-gray-500">{{ __('account.Date_De_Naissance') }}</label>
                            <input type="date" id="date_of_birth" name="date_of_birth" value="{{ old('date_of_birth') }}" class="mt-1 @error('date_of_birth') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                        </div>

                        <div class="col-span-5">
                            <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="{{ __('account.e_mail') }}" autocomplete="email" class="mt-1 block w-full border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm rounded-md @error('email') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                        </div>

                        <div class="col-span-5">
                            <small title="{{ __('account.Password_requirement') }}" class="cursor-help">
                        ...
                        <div class="col-span-5 flex items-center">
                            <input type="password" name="password_confirmation" id="password_confirmation" placeholder="{{ __('account.repeat_Password') }}" autocomplete="password_confirmation" class="mt-1 block w-full border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm rounded-md @error('password') border-red-600 focus:border-red-500 focus:ring-red-500 @enderror">
                    ...
                    <small>
                        {{ __('account.Already_have_an_account') }} <a class="text-blue-500 hover:underline" href="{{ route('ugg.login', app()->getLocale()) }}">{{ __('account.Connectez_vous') }}</a>
                    </small>
                </form>
            </div>
        </div>
